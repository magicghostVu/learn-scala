...
object MState {
    ...
    // thêm logic mới vào trong chuỗi biến đổi
    def modify[S](fModify: S => S): MState[S, Unit] = {
        /*init[S].flatMap[Unit](s => {
            val fForState: S => (S, Unit) = s => (fModify(s), ())
            state[S, Unit](fForState)
        })*/
        val fForState39: S => (S, Unit) = s => (fModify(s), ())
        state(fForState39)
    }
